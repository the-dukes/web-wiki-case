...
console.log(‘run’);
</code></pre><p><img src="`+p+`" alt="image.png"></p><p><strong>案例：定时器</strong></p><blockquote><p>setTimeout 的等待时间结束后并不是直接执行的而是先推入浏览器的一个任务队列，在同步队列结束后在依次调用任务队列中的任务。</p></blockquote><blockquote><p>setTimeout(function(){}, 0) Js 主线程中的执行栈为空时，0 毫秒实际上也达不到的，根据 HTML 标准，最低 4 毫秒。</p></blockquote><blockquote><p>setInterval 是每隔一段时间把任务放到 Event Queue 之中。</p></blockquote><pre><code>function test (num) {
	for (var i = 0; I &lt; num; i++) {
		console.log(i);
	}
}
